...
class LinkedList(abstract):
    ...
    def __next__(self):
        ...
        if not self.pointer:  # check if the current node is none
            raise StopIteration

        data = self.pointer.data  # cache the data
        self.pointer = self.pointer.next  # increment the pointer to next node
        return data  # return the current node data
